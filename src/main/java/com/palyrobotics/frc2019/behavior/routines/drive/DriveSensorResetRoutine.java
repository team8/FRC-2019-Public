...
public class DriveSensorResetRoutine extends Routine {
	...
	public boolean isFinished() {
		if (Timer.getFPGATimestamp() - mStartTime > mTimeout) {
			// Logger.getInstance().logRobotThread(Level.WARNING, "Drive sensor reset
			// routine timed out!");
			return true;
		} else
			return Math.abs(mDrive.getPose().leftEncoderPosition) <= DrivetrainConstants.kAcceptableEncoderZeroError
					&& Math.abs(
							mDrive.getPose().rightEncoderPosition) <= DrivetrainConstants.kAcceptableEncoderZeroError
