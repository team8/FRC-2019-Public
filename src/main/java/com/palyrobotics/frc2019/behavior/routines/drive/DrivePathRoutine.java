...
public class DrivePathRoutine extends Routine {
			...
			for (Path.Waypoint point : mPathList) {
				if (point.isRelative) {
					if (point.marker != null) {
						absoluteList.add(new Path.Waypoint(mRobotState.getLatestFieldToVehicle().getValue()
								.getTranslation().translateBy(point.position), point.speed, point.marker, false));
					} else {
						absoluteList.add(new Path.Waypoint(mRobotState.getLatestFieldToVehicle().getValue()
								.getTranslation().translateBy(point.position), point.speed, false));
					}
				} else {
	...
	public String toString() {
		...
		for (Path.Waypoint wayPoint : path) {
			enumeratedPath.append(wayPoint.position.getX() + offsetX).append(",")
					.append(wayPoint.position.getY() + offsetY).append(",").append(wayPoint.speed).append("\n");
		}
