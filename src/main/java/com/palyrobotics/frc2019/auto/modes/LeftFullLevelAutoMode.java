...
public class LeftFullLevelAutoMode extends AutoModeBase {
	// Left start > rocket ship close > loading station > rocket ship far > depot >
	// rocket ship mid
	...
	public Routine getRoutine() {
		return new SequentialRoutine(new ReZeroSubAutoMode().ReZero(true), placeHatchClose(), takeHatch(),
				placeHatchFar(), takeCargo(), placeCargoMid());
	}
	...
	public Routine placeHatchClose() { // start to rocket ship close
		...
		DepotToRocketShip.add(
				new Waypoint(new Translation2d(kHabLineX + PhysicalConstants.kRobotLengthInches + kOffsetX, 0), 100)); // goes
																														// straight
																														// at
																														// the
																														// start
	...
	public Routine takeHatch() { // rocket ship close to loading station
		...
		CargoShipToLoadingStation
				.add(new Waypoint(
						new Translation2d(kHabLineX - PhysicalConstants.kRobotLengthInches * .5 + kOffsetX,
								kLeftLoadingStationY - PhysicalConstants.kRobotWidthInches * .4 + kOffsetY),
						kRunSpeed)); // lines up with loading station
	...
	public Routine placeHatchFar() { // loading station to rocket ship close
		...
		/*
		 * The robot starts backwards at the loading station after loading a hatch. It
		 * then goes around the rocket ship and over shoots it a bit. Then, it lines up
		 * with the rocket ship far and places the hatch.
		 */

		List<Waypoint> backLoadingStationToRocketShip = new ArrayList<>(); // robot starts going backwards
		backLoadingStationToRocketShip.add(
				new Waypoint(new Translation2d(kLeftLoadingStationX + PhysicalConstants.kRobotLengthInches + kOffsetX,
						kLeftLoadingStationY + kOffsetY), 180)); // backs up a bit
		backLoadingStationToRocketShip.add(new Waypoint(
				new Translation2d(kLeftRocketShipMidX + kOffsetX,
						findLineFar(kLeftRocketShipMidX + PhysicalConstants.kRobotLengthInches * 1.8) + kOffsetY),
				kRunSpeed)); // goes around the rocket ship
		backLoadingStationToRocketShip.add(new Waypoint(
				new Translation2d(kLeftRocketShipFarX + PhysicalConstants.kRobotLengthInches * 1.9 + kOffsetX,
						findLineFar(kLeftRocketShipMidX + PhysicalConstants.kRobotLengthInches * 1.9)
								- PhysicalConstants.kRobotLengthInches * .2 + kOffsetY),
				0));
		routines.add(new DrivePathRoutine(new Path(backLoadingStationToRocketShip), true)); // robot turns and then
																							// moves forward to the
																							// rocket ship

		List<Waypoint> forwardLoadingStationToRocketShip = new ArrayList<>(); // robot turns and then moves forward
		forwardLoadingStationToRocketShip.add(new Waypoint(
				new Translation2d(kLeftRocketShipFarX + PhysicalConstants.kRobotLengthInches * 1.6 + kOffsetX,
						findLineFar(kLeftRocketShipMidX + PhysicalConstants.kRobotLengthInches * 1.6)
								- PhysicalConstants.kRobotLengthInches * .3 + kOffsetY),
				kRunSpeed)); // line up with rocket ship far
		forwardLoadingStationToRocketShip.add(new Waypoint(kLeftRocketShipFar, 0)); // ends in front of the rocket ship
																					// far
		// move elevator up while driving
		// routines.add(new ParallelRoutine(new DrivePathRoutine(new
		// Path(forwardLoadingStationToRocketShip), false),
		// new
		// ElevatorCustomPositioningRoutine(ElevatorConstants.kElevatorRocketHatchHeight1Inches,
		// 1)));
	...
	public Routine takeCargo() { // rocket ship close to depot
		...
		/*
		 * The robot starts at rocket ship far and goes around the rocket backwards.
		 * Ends at the depot and loads a cargo.
		 */

		List<Waypoint> RocketShipToDepot = new ArrayList<>();
		RocketShipToDepot.add(new Waypoint(
				new Translation2d(kLeftRocketShipFarX + PhysicalConstants.kRobotLengthInches * 1.6 + kOffsetX,
						findLineFar(kLeftRocketShipMidX + PhysicalConstants.kRobotLengthInches * 1.6)
								+ PhysicalConstants.kRobotLengthInches * 0.5 + kOffsetY),
				kRunSpeed));
		RocketShipToDepot.add(
				new Waypoint(new Translation2d(kCargoShipLeftFrontX + PhysicalConstants.kRobotLengthInches + kOffsetX,
						kLeftDepotY + PhysicalConstants.kRobotLengthInches * .2 + kOffsetY), kRunSpeed)); // turn around
																											// the
																											// rocket
																											// ship
		...
		// routines.add(new ParallelRoutine(new DrivePathRoutine(new
		// Path(RocketShipToDepot), true),
		/// new ElevatorCustomPositioningRoutine(ElevatorConstants.kBotomPositionInches,
		// 1)));
	...
	public Routine placeCargoMid() { // depot to rocket ship mid
		...
		DepotToRocketShipMid
				.add(new Waypoint(new Translation2d(kLeftDepotX + PhysicalConstants.kRobotLengthInches * 2 + kOffsetX,
						kLeftDepotY + kOffsetY), kRunSpeed)); // go straight until near rocket ship
		DepotToRocketShipMid
				.add(new Waypoint(new Translation2d(kLeftRocketShipMidX - PhysicalConstants.kRobotLengthInches * .3,
						kLeftDepotY + kOffsetY), kRunSpeed));
		DepotToRocketShipMid.add(new Waypoint(
				new Translation2d(kLeftRocketShipMidX, kLeftRocketShipMidY - PhysicalConstants.kRobotLengthInches * .8),
				kRunSpeed)); // line up with rocket ship
		DepotToRocketShipMid.add(new Waypoint(new Translation2d(kLeftRocketShipMidX, kLeftRocketShipMidY), 0));
		// move elevator up while driving
		// routines.add(new ParallelRoutine(new DrivePathRoutine(new
		// Path(DepotToRocketShipMid), false),
		// new
		// ElevatorCustomPositioningRoutine(ElevatorConstants.kElevatorRocketCargoHeight1Inches,
		// 1)));
	...
	public double findLineClose(double cordX) {
		return 0.54862 * cordX - 0.54862 * kLeftRocketShipCloseX + kLeftRocketShipCloseY; // slope is derived from the
																							// angle of the rocket ship
																							// sides - constants derived
																							// from math
	} // the y cord of an invisible line extending from rocket ship close

	public double findLineFar(double cordX) {
		return -0.54862 * cordX + 0.54862 * kLeftRocketShipFarX + kLeftRocketShipFarY; // slope is derived from the
																						// angle of the rocket ship
																						// sides - constants derived
																						// from math
	} // the y cord of an invisible line extending from rocket ship close
