...
public class RightFullLevelAutoMode extends AutoModeBase {
	...
	public Routine placeHatchClose() { // start to rocket ship close
		...
		DepotToRocketShip.add(new Waypoint(
				new Translation2d(kHabLineX + PhysicalConstants.kRobotLengthInches + kOffsetX, 0), kRunSpeed)); // goes
																												// straight
																												// at
																												// the
																												// start
		...
		routines.add(new ParallelRoutine(new DrivePathRoutine(goForward, false, true), new PusherOutRoutine()));
	...
	public Routine takeHatch() { // rocket ship close to loading station
		...
		BackCargoShipToLoadingStation.add(new Waypoint(kRightLoadingStation.translateBy(new Translation2d(
				PhysicalConstants.kRobotLengthInches * 2, -PhysicalConstants.kRobotLengthInches * 0.5)), 0));
		...
		ForwardCargoShipToLoadingStation.add(new Waypoint(
				kRightLoadingStation.translateBy(new Translation2d(PhysicalConstants.kRobotLengthInches * 1.5, 0)),
				kRunSpeed));
	...
	public Routine placeHatchFar() { // loading station to rocket ship close
		...
		backLoadingStationToRocketShip.add(
				new Waypoint(new Translation2d(kRightLoadingStationX + PhysicalConstants.kRobotLengthInches + kOffsetX,
						kRightLoadingStationY + kOffsetY), kRunSpeed)); // backs up a bit
		backLoadingStationToRocketShip.add(new Waypoint(
				new Translation2d(kRightRocketShipMidX + kOffsetX,
						findLineFar(kRightRocketShipMidX + PhysicalConstants.kRobotLengthInches * 1.8) + kOffsetY),
				kRunSpeed)); // goes around the rocket ship
		backLoadingStationToRocketShip.add(new Waypoint(
				new Translation2d(kRightRocketShipFarX + PhysicalConstants.kRobotLengthInches * 1.9 + kOffsetX,
						findLineFar(kRightRocketShipMidX + PhysicalConstants.kRobotLengthInches * 1.9)
		...
		forwardLoadingStationToRocketShip.add(new Waypoint(
				new Translation2d(kRightRocketShipFarX + PhysicalConstants.kRobotLengthInches * 1.6 + kOffsetX,
						findLineFar(kRightRocketShipMidX + PhysicalConstants.kRobotLengthInches * 1.6)
								- PhysicalConstants.kRobotLengthInches * .3 + kOffsetY),
				kRunSpeed)); // line up with rocket ship far
		forwardLoadingStationToRocketShip.add(new Waypoint(kRightRocketShipFar, 0)); // ends in front of the rocket ship
																						// far
	...
	public Routine takeCargo() { // rocket ship close to depot - could be more accurate
		...
		RocketShipToDepot.add(
				new Waypoint(new Translation2d(kCargoShipRightFrontX + PhysicalConstants.kRobotLengthInches + kOffsetX,
						kRightDepotY + PhysicalConstants.kRobotLengthInches * .2 + kOffsetY), 180)); // turn around the
																										// rocket ship
	...
	public Routine placeCargoMid() { // depot to rocket ship mid
		...
		DepotToRocketShipMid
				.add(new Waypoint(new Translation2d(kRightDepotX + PhysicalConstants.kRobotLengthInches * 2 + kOffsetX,
						kRightDepotY + kOffsetY), kRunSpeed)); // go straight until near rocket ship
		DepotToRocketShipMid
				.add(new Waypoint(new Translation2d(kRightRocketShipMidX - PhysicalConstants.kRobotLengthInches * .4,
						kRightDepotY + kOffsetY), kRunSpeed));
		DepotToRocketShipMid.add(new Waypoint(
				kRightRocketShipMid.translateBy(new Translation2d(0, PhysicalConstants.kRobotLengthInches * .6)),
	...
	public double findLineClose(double cordX) {
		return -0.54862 * cordX + 0.54862 * kRightRocketShipCloseX + kRightRocketShipCloseY; // slope is derived from
																								// the angle of the
																								// rocket ship sides -
																								// constants derived
																								// from math
	...
	public double findLineFar(double cordX) {
		return 0.54862 * cordX - 0.54862 * kRightRocketShipFarX + kRightRocketShipFarY; // slope is derived from the
