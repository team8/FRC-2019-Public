...
public class LeftStartCloseHatchTwoCargoSideAutoMode extends AutoModeBase {
	...
	public Routine getRoutine() {
		return new SequentialRoutine(new LeftStartLeftFrontCargoAutoMode().placeHatch(), CargoShipToDepot(),
				placeCargo(0), takeCargo(1), placeCargo(1));
	}
	...
	public Routine CargoShipToDepot() { // cargo ship to depot
		...
		CargoShipToDepot
				.add(new Waypoint(new Translation2d(kHabLineX + PhysicalConstants.kRobotLengthInches * 1.05 + kOffsetX,
						kLeftDepotY - PhysicalConstants.kRobotLengthInches * .25 + kOffsetY), kRunSpeed)); // line up
																											// with
																											// depot
		CargoShipToDepot
				.add(new Waypoint(new Translation2d(kLeftDepotX + PhysicalConstants.kRobotLengthInches * 1.1 + kOffsetX,
	...
	public Routine placeCargo(int CargoSlot) { // depot to cargo ship bays
		...
		List<Path.Waypoint> DepotToCargoShip = new ArrayList<>(); // the CargoSlot variable makes the robot go farther
																	// so it goes to a different bay each time
		DepotToCargoShip
		...
		DepotToCargoShip
				.add(new Waypoint(new Translation2d(kLeftFirstCargoShipX + PhysicalConstants.kRobotLengthInches * .55
						+ CargoSlot * PhysicalConstants.kCargoLineGap + kOffsetX, kLeftDepotY + kOffsetY), kRunSpeed));
		DepotToCargoShip.add(new Waypoint(new Translation2d(
				kLeftFirstCargoShipX + PhysicalConstants.kRobotLengthInches * .85
						+ CargoSlot * PhysicalConstants.kCargoLineGap + kOffsetX,
				kLeftFirstCargoShipY + PhysicalConstants.kRobotLengthInches + kOffsetY), kRunSpeed)); // line up in
																										// front of
																										// cargo bay
		DepotToCargoShip.add(new Waypoint(new Translation2d(
				kLeftFirstCargoShipX + PhysicalConstants.kRobotLengthInches * .85
	...
	public Routine takeCargo(int DepotSlot) { // cargo ship bays to depot
		...
		List<Path.Waypoint> CargoShipToDepot = new ArrayList<>(); // the DepotSlot variable makes the robot go farther
																	// each time to collect the next cargo
		CargoShipToDepot
				.add(new Waypoint(
						new Translation2d(
								kLeftFirstCargoShipX + DepotSlot * PhysicalConstants.kCargoLineGap
										+ PhysicalConstants.kRobotLengthInches + kOffsetX,
								kLeftFirstCargoShipY + PhysicalConstants.kRobotLengthInches * .7 + kOffsetY),
						kRunSpeed));
		CargoShipToDepot
				.add(new Waypoint(new Translation2d(kLeftFirstCargoShipX + DepotSlot * PhysicalConstants.kCargoLineGap
						+ PhysicalConstants.kRobotLengthInches + kOffsetX, kLeftDepotY + kOffsetY), kRunSpeed)); // turn
																													// back
																													// and
																													// line
																													// up
		...
		CargoShipToDepot
				.add(new Waypoint(
						new Translation2d(kLeftFirstCargoShipX + DepotSlot * PhysicalConstants.kCargoLineGap
								- PhysicalConstants.kRobotLengthInches * .5 + kOffsetX, kLeftDepotY + kOffsetY),
						kRunSpeed));
		CargoShipToDepot.add(new Waypoint(new Translation2d(kHabLineX + kOffsetX, kLeftDepotY + kOffsetY), kRunSpeed));
		CargoShipToDepot.add(new Waypoint(new Translation2d(
				kLeftDepotX + PhysicalConstants.kRobotLengthInches - (DepotSlot + 1) * kCargoDiameter + kOffsetX,
				kLeftDepotY + kOffsetY), 0));
