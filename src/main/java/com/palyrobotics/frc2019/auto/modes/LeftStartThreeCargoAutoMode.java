...
public class LeftStartThreeCargoAutoMode extends AutoModeBase {
	...
	public Routine getRoutine() {
		return new SequentialRoutine(new ReZeroSubAutoMode().ReZero(false), placeCargoStart(), placeCargo(1),
				takeCargo(1), placeCargo(2));
	}
	...
	public Routine placeCargoStart() { // start to cargo ship front
		...
		StartToCargoShip.add(new Waypoint(
				new Translation2d(kHabLineX + PhysicalConstants.kRobotLengthInches + kOffsetX, 0), SPEED * .5));
		StartToCargoShip.add(new Waypoint(
				new Translation2d(kLeftFirstCargoShipX - PhysicalConstants.kRobotLengthInches * 2 + kOffsetX,
						kLeftFirstCargoShipY + PhysicalConstants.kRobotLengthInches + kOffsetY),
				SPEED));
		StartToCargoShip.add(new Waypoint(
				new Translation2d(kLeftFirstCargoShipX + PhysicalConstants.kRobotLengthInches * .5 + kOffsetX,
						kLeftFirstCargoShipY + PhysicalConstants.kRobotLengthInches + kOffsetY),
				0));
		// move elevator up while driving
		// routines.add(new ParallelRoutine(new DrivePathRoutine(new
		// Path(StartToCargoShip), true),
		// new
		// ElevatorCustomPositioningRoutine(ElevatorConstants.kElevatorCargoBaysHeightInches,
		// 1)));
		...
		CargoShipToDepot
				.add(new Waypoint(new Translation2d(kHabLineX + PhysicalConstants.kRobotLengthInches * 1 + kOffsetX,
	...
	public Routine placeCargo(int CargoSlot) {
		...
		DepotToCargoShip.add(
				new Waypoint(new Translation2d(kLeftFirstCargoShipX * .4 + kOffsetX, kLeftDepotY + kOffsetY), SPEED));
		DepotToCargoShip.add(new Waypoint(new Translation2d(
				kLeftFirstCargoShipX - PhysicalConstants.kRobotLengthInches * 2
		...
		DepotToCargoShip.add(new Waypoint(new Translation2d(
				kLeftFirstCargoShipX + PhysicalConstants.kRobotLengthInches * .5
						+ CargoSlot * PhysicalConstants.kCargoLineGap + kOffsetX,
				kLeftFirstCargoShipY + PhysicalConstants.kRobotLengthInches + kOffsetY), 0));
		// move elevator up while driving
		// routines.add(new ParallelRoutine(new DrivePathRoutine(new
		// Path(DepotToCargoShip), true),
		// new
		// ElevatorCustomPositioningRoutine(ElevatorConstants.kElevatorCargoBaysHeightInches,
		// 1)));
	...
	public Routine takeCargo(int DepotSlot) {
		...
		CargoShipToDepot.add(
				new Waypoint(new Translation2d(kLeftFirstCargoShipX * .5 + kOffsetX, kLeftDepotY + kOffsetY), SPEED));
		CargoShipToDepot.add(new Waypoint(
				new Translation2d(kHabLineX + PhysicalConstants.kRobotLengthInches + kOffsetX, kLeftDepotY + kOffsetY),
				SPEED));
		CargoShipToDepot.add(new Waypoint(new Translation2d(
				kLeftDepotX + PhysicalConstants.kRobotLengthInches - DepotSlot * kCargoDiameter + kOffsetX,
				kLeftDepotY + kOffsetY), 0));
		// move elevator down while driving
		// routines.add(new ParallelRoutine(new DrivePathRoutine(new
		// Path(CargoShipToDepot), true),
