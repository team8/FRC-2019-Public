...
public class test1 extends AutoModeBase {
	...
	public Routine getRoutine() {
		...
		path30.add(new Path.Waypoint(new Translation2d(0.0, 5643.0), 0.0));
		...
		return new SequentialRoutine(
				new ParallelRoutine(new DrivePathRoutine(new Path(path000), false),
						new DrivePathRoutine(new Path(path010), false)),
				new BBTurnAngleRoutine(36.0), new BBTurnAngleRoutine(369.0),
				new DrivePathRoutine(new Path(path30), false),
				new DriveTimeRoutine(10000, new SparkDriveSignal(left41, right41)),
				new ShooterExpelRoutine(Shooter.ShooterState.SPIN_UP, 100.0));
	}
