...
public class LazySparkMax extends CANSparkMax {
	...
	private static Map<ControlType, Integer> sControlTypeToSlot = Map.of(ControlType.kSmartMotion, 1,
	...
	public boolean set(ControlType type, double reference, double arbitraryPercentOutput, Gains gains) {
		...
		if (!areGainsEqual || slot != mLastSlot || type != mLastControlType || reference != mLastReference
				|| arbitraryPercentOutput != mLastArbitraryPercentOutput) {
			if (getPIDController().setReference(reference, type, slot, arbitraryPercentOutput,
				...
				DriverStation.reportError(
						String.format("Error updating output on spark max with ID: %d", getDeviceId()),
	...
	private void updateGainsIfNeeded(Gains gains, int slot) {
		if (gains != null) {
			...
			if (Double.compare(lastGains.p, gains.p) != 0)
				controller.setP(gains.p, slot);
			if (Double.compare(lastGains.i, gains.i) != 0)
				controller.setI(gains.i, slot);
			if (Double.compare(lastGains.d, gains.d) != 0)
				controller.setD(gains.d, slot);
			if (Double.compare(lastGains.f, gains.f) != 0)
				controller.setFF(gains.f, slot);
			if (Double.compare(lastGains.iZone, gains.iZone) != 0)
				controller.setIZone(gains.iZone, slot);
			if (gains instanceof SmartGains) {
				...
				if (Double.compare(lastSmartGains.acceleration * mRobotConfig.smartMotionMultiplier,
						smartGains.acceleration * mRobotConfig.smartMotionMultiplier) != 0)
					controller.setSmartMotionMaxAccel(smartGains.acceleration * mRobotConfig.smartMotionMultiplier,
							slot);
				if (Double.compare(lastSmartGains.velocity * mRobotConfig.smartMotionMultiplier,
						smartGains.velocity * mRobotConfig.smartMotionMultiplier) != 0)
					controller.setSmartMotionMaxVelocity(smartGains.velocity * mRobotConfig.smartMotionMultiplier,
							slot);
