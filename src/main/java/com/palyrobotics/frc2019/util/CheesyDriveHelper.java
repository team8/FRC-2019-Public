...
public class CheesyDriveHelper {
	...
	private double mOldWheel, mPreviousWheelForRamp, mPreviousThrottleForRamp, mQuickStopAccumulator,
			mNegativeInertiaAccumulator, mBrownOutTimeSeconds;
	...
	public SparkDriveSignal cheesyDrive(Commands commands, RobotState robotState) {
		...
		if (RobotController.isBrownedOut()) {
			...
			totalPowerMultiplier = MathUtil.clamp(
					mDriveConfig.brownOutInitialNerfMultiplier + (Timer.getFPGATimestamp() - mBrownOutTimeSeconds)
							* (1 / mDriveConfig.brownOutRecoverySeconds)
							* (1 - mDriveConfig.brownOutInitialNerfMultiplier),
		...
		if (absoluteThrottle > mDriveConfig.throttleAccelerationThreshold
				&& absoluteThrottle > Math.abs(mPreviousThrottleForRamp)) {
		...
		if (isQuickTurn && absoluteWheel > mDriveConfig.wheelAccelerationThreshold
				&& absoluteWheel > Math.abs(mPreviousWheelForRamp)) {
