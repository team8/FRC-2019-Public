	...
	public Path deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException, JsonProcessingException {
		...
		for (var i = 0; i < jsonSplit.length; i++) {
			...
			if (jsonSplit[i].equals("x")) {
				...
				pathList.set(currentWaypoint,
						new Path.Waypoint(new Translation2d(targetX, pathList.get(currentWaypoint).position.getY()),
								pathList.get(currentWaypoint).speed));
			}
			// same for y
			if (jsonSplit[i].equals("y")) {
				double targetY = Double.parseDouble(jsonSplit[i - 1]);
				pathList.set(currentWaypoint,
						new Path.Waypoint(new Translation2d(pathList.get(currentWaypoint).position.getX(), targetY),
								pathList.get(currentWaypoint).speed));
			}
			// same for speed
			if (jsonSplit[i].equals("speed")) {
				...
				pathList.set(currentWaypoint,
						new Path.Waypoint(new Translation2d(pathList.get(currentWaypoint).position.getX(),
