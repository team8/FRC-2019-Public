 ...
 * <p>
 * Various changes to path construction were made in accordance with team 1712's
 * whitepaper: https://www.chiefdelphi.com/media/papers/3488? These improvements
 * are represented by the following methods: {@link #injectPoints(List)}
 * injectPoints} {@link #smoothOut(List) smoothen} {@link #setVelocities(List)
 * setVelocities}
 * {@link #getCurvature(Translation2d, Translation2d, Translation2d)
 * getCurvature}
 *
...
public class Path {
	...
	public Path(List<Waypoint> wayPoints) {
		...
		if (mWaypoints.size() > 1) {
			...
			if (firstWaypoint.marker != null)
				mMarkersCrossed.add(firstWaypoint.marker);
			mWaypoints.remove(0);
		}
	}
	...
	// Add intermediate, closely-spaced points between wayPoints to improve
	// following
	private void injectPoints(List<Waypoint> wayPoints) {
		...
		for (int i = 0; i < wayPoints.size() - 1; i++) {
			...
			// Portion of the segment's length occupied by one interval; needed for the
	...
	// Smooth the path using gradient descent; inspired by
	...
	private void smoothOut(List<Waypoint> wayPoints) {
		...
		// Sum of the horizontal and vertical shifts of each point in one iteration of
		// smoothing
		double change = DrivetrainConstants.kSmoothingTolerance;
		int numIters = 0;
		// Perform smoothing passes until convergence
		while (change >= DrivetrainConstants.kSmoothingTolerance
				&& numIters <= DrivetrainConstants.kSmoothingMaxIters) {
			change = 0.0;
			// Smooth all points except for the final one
			// We deleted the first waypoint at (0, 0) but for smoothing code to work we
			// have to pretend it's still there
			for (int i = 0; i < oldPts.size() - 1; i++) {
				...
				double changeX = DrivetrainConstants.kSmoothingWeightData * (x4 - x2)
						+ DrivetrainConstants.kSmoothingWeight * (x1 + x3 - (2.0 * x2));
				double changeY = DrivetrainConstants.kSmoothingWeightData * (y4 - y2)
						+ DrivetrainConstants.kSmoothingWeight * (y1 + y3 - (2.0 * y2));
				x2 += changeX;
				y2 += changeY;
	...
	/*
	 * Automatically calculate target velocities throughout the path. For each
	 * waypoint before the last one, the velocity is the smaller value between
	 * Constants.kPathFollowingMaxVel and k/curvature, where k is an arbitrary
	 * constant.
	 */
	...
	private void setVelocities(List<Waypoint> wayPoints) {
		for (int i = 0; i < wayPoints.size() - 1; i++) {
			...
			if (wayPoints.get(i).speed == -1) {
				...
				double maxVel = Math.min(DrivetrainConstants.kPathFollowingMaxVel,
						DrivetrainConstants.kTurnVelocityReduction / getCurvature(previous, current, next));
	...
	/*
	 * Determine curvature of the circular arc connecting the current waypoint with
	 * those directly before and after it Used for curvature calculation and
	 * adjustments to velocity around turns
	 * @param previous - the first waypoint in the path to be connected
	 * @param current - the second waypoint to be connected. The curvature
	 * calculation applies to this point
	 * @param next - the third waypoint in the path to be connected
	 ...
	 * @return A segment of the robot's predicted motion with start/end points and
	 *         speed.
	 */
	public PathSegment.Sample getLookaheadPoint(Translation2d position, double lookAheadDistance) {
		...
		for (PathSegment segment : mSegments) {
			...
			if (distance >= lookAheadDistance) {
				...
				Optional<Translation2d> intersectionPoint = getFirstCircleSegmentIntersection(segment, position,
						lookAheadDistance);
		...
		PathSegment newLastSegment = new PathSegment(lastSegment.getStart(), lastSegment.interpolate(10000),
				lastSegment.getSpeed());
		Optional<Translation2d> intersectionPoint = getFirstCircleSegmentIntersection(newLastSegment, position,
				lookAheadDistance);
		return intersectionPoint.map(translation2d -> new PathSegment.Sample(translation2d, lastSegment.getSpeed()))
				.orElseGet(() -> new PathSegment.Sample(lastSegment.getEnd(), lastSegment.getSpeed()));
	}

	private static Optional<Translation2d> getFirstCircleSegmentIntersection(PathSegment segment, Translation2d center,
			double radius) {
