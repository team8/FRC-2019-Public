...
public class DriveConfig extends AbstractSubsystemConfig {
	...
	// kA = (wheel radius * robot mass) / (total number of motors * gear reduction *
	// motor stall torque)
	...
	public double throttleAccelerationLimit, wheelAccelerationLimit, throttleAccelerationThreshold,
			wheelAccelerationThreshold, quickStopWeight, quickTurnScalar, quickStopDeadBand, quickStopScalar,
			turnSensitivity, lowNegativeInertiaThreshold, lowNegativeInertiaFarScalar, lowNegativeInertiaCloseScalar,
			lowNegativeInertiaTurnScalar, wheelNonLinearity, controllerRampRate, brownOutRecoverySeconds,
			brownOutInitialNerfMultiplier, velocityUnlock, initialLock;
