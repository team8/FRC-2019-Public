...
// return String.format("dt: %s, max_acc: %s, max_vel: %s", dT, maxAcceleration,
// maxVelocity);
// }
// }
//
...
// double max_reachable_velocity_disc = cur_vel2 / 2.0 + config_.maxAcceleration
// * distance_to_go;
// double min_reachable_velocity_disc = cur_vel2 / 2.0 - config_.maxAcceleration
// * distance_to_go;
// double cruise_vel = cur_vel;
// if (min_reachable_velocity_disc < 0 || cruise_vel < 0) {
// cruise_vel = Math.min(config_.maxVelocity,
// Math.sqrt(max_reachable_velocity_disc));
// }
// double t_start = (cruise_vel - cur_vel) / config_.maxAcceleration;
// //Accelerate
// //to
// //cruise_vel
// double x_start = cur_vel * t_start + .5 * config_.maxAcceleration * t_start *
// t_start;
// double t_end = Math.abs(cruise_vel / config_.maxAcceleration); //Decelerate
// //to zero
// //vel.
// double x_end = cruise_vel * t_end - .5 * config_.maxAcceleration * t_end *
// t_end;
...
// next_state_.position = x_start + x_cruise + cruise_vel * delta_t - .5 *
// config_.maxAcceleration * delta_t * delta_t;
...
// error_sum_ = 0;
// }
// double output = kp_ * error + kd_ * ((error - last_error_) / dt -
// set_point_.vel) + (kv_ * set_point_.vel + ka_ * set_point_.acc);
...
// return Math.abs(set_point_.position - goal_position_) < 1E-3 &&
// Math.abs(set_point_.vel) < 1E-2;
// }
//
