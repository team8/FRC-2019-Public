...
public class OperatorInterface {
    ...
    private final Joystick mDriveStick = HardwareAdapter.getInstance().getJoysticks().driveStick,
            mTurnStick = HardwareAdapter.getInstance().getJoysticks().turnStick;
    private final XboxController mOperatorXboxController = HardwareAdapter.getInstance()
            .getJoysticks().operatorXboxController;

    // Timestamp when a vision routine was last activated; helps us know when to
    // turn LEDs off
    ...
    Commands updateCommands(Commands commands) {
         ...
         * Drivetrain controls
         */
        if (commands.wantedDriveState != Drive.DriveState.OFF_BOARD_CONTROLLER
                && commands.wantedDriveState != Drive.DriveState.ON_BOARD_CONTROLLER) {
            commands.wantedDriveState = Drive.DriveState.CHEZY;
        }
        // More safety
        if (Math.abs(mDriveStick.getY()) > DrivetrainConstants.kDeadband
                || Math.abs(mTurnStick.getX()) > DrivetrainConstants.kDeadband) {
            commands.wantedDriveState = Drive.DriveState.CHEZY;
        }
        ...
        // }
        //
        // if (mOperatorXboxController.getButtonX() && commands.wantedShovelUpDownState
        // == Shovel.UpDownState.UP && (mLastCancelTime + 200) <
        ...
        // (System.currentTimeMillis() - 450 > mIntakeStartTime) &&
        ...
        if (Math.abs(rightStick) > 0.1) {
            ...
            commands.customElevatorVelocity = -rightStick * elevatorConfig.gains.velocity
                    * elevatorConfig.manualPowerMultiplier;
        }
        if (mOperatorXboxController.getAButtonPressed()) { // Level 1
            ...
            commands.cancelCurrentRoutines = false;
            commands.addWantedRoutine(new SequentialRoutine(new PusherInRoutine(), new IntakeUpRoutine(),
                    new WaitForElevatorCanMove(), elevatorLevel1));
        } else if (mOperatorXboxController.getBButtonPressed()) { // Level 2
            double levelHeight = Robot.getRobotState().hasPusherCargoFar ? elevatorConfig.elevatorCargoHeight2
            ...
            commands.cancelCurrentRoutines = false;
            commands.addWantedRoutine(new SequentialRoutine(new PusherInRoutine(), new IntakeUpRoutine(),
                    new WaitForElevatorCanMove(), elevatorLevel2));
            ...
            commands.cancelCurrentRoutines = false;
            commands.addWantedRoutine(new SequentialRoutine(new PusherInRoutine(), new IntakeUpRoutine(),
                    new WaitForElevatorCanMove(), elevatorLevel3));
        }
        ...
        } else if (!mOperatorXboxController.getRightTrigger()
                && commands.wantedIntakeState == Intake.IntakeMacroState.EXPELLING_CARGO) {
            commands.wantedIntakeState = Intake.IntakeMacroState.HOLDING_CARGO;
        }
        if (mOperatorXboxController.getLeftTrigger()
            ...
            commands.wantedIntakeState = Intake.IntakeMacroState.INTAKING_CARGO;
                ...
                && commands.wantedIntakeState == IntakeMacroState.INTAKING_CARGO) {
            commands.wantedIntakeState = Intake.IntakeMacroState.HOLDING_CARGO;
        }
