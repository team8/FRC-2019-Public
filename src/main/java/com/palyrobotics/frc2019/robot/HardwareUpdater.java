...
class HardwareUpdater {
	...
	private void startUltrasonics() {
		...
		// Ultrasonic pusherSecondaryUltrasonic =
		// HardwareAdapter.getInstance().getPusher().pusherSecondaryUltrasonic;
	...
	void updateState(RobotState robotState) {
		...
		// robotState.drivePose.headingVelocity = (robotState.drivePose.heading -
		// robotState.drivePose.lastHeading) / DrivetrainConstants.kNormalLoopsDt;

		// robotState.drivePose.lastLeftEncoderPosition =
		// robotState.drivePose.leftEncoderPosition;
		// robotState.drivePose.leftEncoderPosition =
		// drivetrain.leftMasterSpark.getEncoder().getPosition();
		// robotState.drivePose.leftEncoderVelocity =
		// drivetrain.leftMasterSpark.getEncoder().getVelocity();
		// robotState.drivePose.lastRightEncoderPosition =
		// robotState.drivePose.rightEncoderPosition;
		// robotState.drivePose.rightEncoderPosition =
		// drivetrain.rightMasterSpark.getEncoder().getPosition();
		// robotState.drivePose.rightEncoderVelocity =
		// drivetrain.rightMasterSpark.getEncoder().getVelocity();

		// double robotVelocity = (robotState.drivePose.leftEncoderVelocity +
		// robotState.drivePose.rightEncoderVelocity) / 2;
	...
	private void updateUltrasonicSensors(RobotState robotState) {
		...
		robotState.hasIntakeCargo = hasCargoFromReadings(robotState.leftIntakeReadings, intakeConfig.cargoInchTolerance,
				intakeConfig.cargoCountRequired)
				|| hasCargoFromReadings(robotState.rightIntakeReadings, intakeConfig.cargoInchTolerance,
						intakeConfig.cargoCountRequired);
		...
		robotState.hasPusherCargo = hasCargoFromReadings(robotState.pusherReadings, pusherConfig.cargoTolerance,
				OtherConstants.kRequiredUltrasonicCount + 1);
		robotState.hasPusherCargoFar = hasCargoFromReadings(robotState.pusherReadings, pusherConfig.cargoToleranceFar,
				OtherConstants.kRequiredUltrasonicCount);

		if (lastHasPusherCargoFar != robotState.hasPusherCargoFar) {
			int properPipeline = robotState.hasIntakeCargo ? OtherConstants.kLimelightCargoPipeline
					: OtherConstants.kLimelightHatchPipeline;
	...
	/**
	 * Updates the drivetrain Uses SparkOutput and can run off-board control loops
	 * through SRX
	 */
	private void updateDrivetrain() {
		updateSparkMax(HardwareAdapter.getInstance().getDrivetrain().leftMasterSpark,
				mDrive.getDriveSignal().leftOutput);
		updateSparkMax(HardwareAdapter.getInstance().getDrivetrain().rightMasterSpark,
				mDrive.getDriveSignal().rightOutput);
		// CSVWriter.addData("leftActualPower",
		// HardwareAdapter.getInstance().getDrivetrain().leftMasterSpark.getAppliedOutput());
		// CSVWriter.addData("rightActualPower",
		// HardwareAdapter.getInstance().getDrivetrain().rightMasterSpark.getAppliedOutput());
		// System.out.println("HardwareAdapter.getInstance().getDrivetrain().leftMasterSpark
		// = " +
		// HardwareAdapter.getInstance().getDrivetrain().leftMasterSpark.getAppliedOutput());
		// System.out.println("HardwareAdapter.getInstance().getDrivetrain().rightMasterSpark
		// = " +
		// HardwareAdapter.getInstance().getDrivetrain().rightMasterSpark.getAppliedOutput());
	}
	...
	/**
	 * Runs the compressor only when the pressure too low or the current draw is low
	 * enough
	 */
	private boolean shouldCompress() {
		return RobotState.getInstance().gamePeriod != RobotState.GamePeriod.AUTO
				&& !RobotState.getInstance().isQuickTurning;
	}

	private boolean shouldRumble() {
		boolean rumble;
		double intakeRumbleLength = mIntake.getRumbleLength(), shooterRumbleLength = mShooter.getRumbleLength();
	...
	// if (output.getControlMode().equals(ControlMode.Position) ||
	// output.getControlMode().equals(ControlMode.Velocity)
	...
	// talon.config_IntegralZone(output.profile, (int)
	// Math.round(output.gains.iZone), 0);
	...
	// }
	// if (output.getArbitraryFF() != 0.0 &&
	// output.getControlMode().equals(ControlMode.Position)) {
	// talon.set(output.getControlMode(), output.getSetPoint(),
	// DemandType.ArbitraryFeedForward, output.getArbitraryFF());
	// } else {
	// talon.set(output.getControlMode(), output.getSetPoint(), DemandType.Neutral,
	// 0.0);
	// }
	// }
