...
public class DriveStraightController implements DriveController {
	...
	public boolean onTarget() {
		...
		return Math.abs(Robot.getRobotState().drivePose.heading) < kTolerance
				&& Math.abs((Robot.getRobotState().drivePose.leftEncoderPosition
						+ Robot.getRobotState().drivePose.rightEncoderPosition) / 2 - mTarget) < kTolerance
				&& Math.abs(Robot.getRobotState().drivePose.leftEncoderVelocity) < 0.05
				&& Math.abs(Robot.getRobotState().drivePose.rightEncoderVelocity) < 0.05;
	}
	...
	public SparkDriveSignal update(RobotState state) {
		...
		// double turn = headingPID.calculate(state.drivePose.heading) *
		// Constants.kDriveInchesPerDegree;
		double turn = 0;
