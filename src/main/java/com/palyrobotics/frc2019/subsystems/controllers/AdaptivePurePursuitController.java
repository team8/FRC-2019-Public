...
public class AdaptivePurePursuitController implements Drive.DriveController {
	...
	public AdaptivePurePursuitController(double fixedLookAhead, double maxAcceleration, double nominalDt, Path path, boolean reversed, double pathCompletionTolerance) {
		...
		mOnBoardController = new OnBoardDriveController(OnBoardControlType.kVelocityWithArbitraryDemand,
				Configs.get(DriveConfig.class).trajectoryGains);
	}
	...
	public SparkDriveSignal update(RobotState state) {
		...
		if (mReversed) {
			robotPose = new RigidTransform2d(robotPose.getTranslation(),
					robotPose.getRotation().rotateBy(Rotation2d.fromRadians(Math.PI)));
		}

		double distanceFromPath = mPath.update(robotPose.getTranslation());
		PathSegment.Sample lookAheadPoint = mPath.getLookaheadPoint(robotPose.getTranslation(),
				distanceFromPath + mFixedLookahead);
		...
		// If we are just starting the controller, use the robot's current velocity and
		// the default dt
		double now = Timer.getFPGATimestamp();
		double dt = now - mLastTime;
		if (mLastDriveVelocity == null || mLastCommand == null) {
			mLastDriveVelocity = new Kinematics.DriveVelocity(state.drivePose.leftEncoderVelocity,
		...
		if (circle.isPresent()) {
			command = new RigidTransform2d.Delta(speed, 0,
					(circle.get().turnRight ? -1 : 1) * Math.abs(speed) / circle.get().radius);
		} else {
	...
	private static Optional<Circle> joinPath(RigidTransform2d robotPose, Translation2d lookAheadPoint) {
		...
		double crossProduct = poseToLookahead.getX() * robotPose.getRotation().sin()
				- poseToLookahead.getY() * robotPose.getRotation().cos();
