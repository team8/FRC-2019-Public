...
/**
 * Represents the drivetrain Uses controllers or
 * cheesydrivehelper/proportionaldrivehelper to calculate DriveSignal
 *
 * @author Not Nihar
 */
public class Drive extends Subsystem {
	 ...
	 * {@code ON_BOARD_CONTROLLER} makes a control loop calculated in code with an
	 * open loop. It uses drive outputs passed in through commands. {@code NEUTRAL}
	 * does nothing.
	 */
	 ...
	 *
	 * <br>
	 * Contains a state machine that switches. based on {@link DriveState} and
	 * updates the {@link DriveController} with the current {@link RobotState}. The
	 * controllers then output a {@link SparkDriveSignal}, which is then used to
	 * {@link Drive#setDriveOutputs}. <br>
	 * <br>
	 * <p>
	 * States and behavior:
	 * <ul>
	 * <li>{@link DriveState#CHEZY}: Sets drive outputs using a
	 * {@link SparkDriveSignal} from a {@link CheesyDriveHelper}.</li>
	 * <li>{@link DriveState#OFF_BOARD_CONTROLLER}: Updates the
	 * {@link DriveController}, but if no controller exists prints a warning and
	 * sets the {@link Commands#wantedDriveState} to {@link DriveState#NEUTRAL}.
	 * </li>
	 * <li>{@link DriveState#ON_BOARD_CONTROLLER}: See
	 * {@link DriveState#OFF_BOARD_CONTROLLER}.</li>
	 * <li>{@link DriveState#OPEN_LOOP}: Directly accesses
	 * {@link Commands#robotSetPoints} for the {@code DrivePowerSetpoint}.</li>
	 * <li>{@link DriveState#NEUTRAL}: Sets drive outputs to a blank
	 * {@link SparkDriveSignal}. Will also {@link Drive#resetController} if in a new
	 * state and set {@link DriveState#CHEZY} if in {@code TELEOP}.</li>
	 * </ul>
	 *
	...
	public void update(Commands commands, RobotState state) {
		...
		switch (mState) {
			...
			case ON_BOARD_CONTROLLER:
				if (mController == null) {
					// Logger.getInstance().logSubsystemThread(Level.WARNING, "No onboard controller
					// to use!");
		...
		// CSVWriter.addData("driveLeftEncVelocity",
		// state.drivePose.leftEncoderVelocity);
		// CSVWriter.addData("driveRightEnc", state.drivePose.rightEncoderPosition);
		// CSVWriter.addData("driveRightEncVelocity",
		// state.drivePose.rightEncoderVelocity);
	...
	public void setTrajectoryController(Path path, boolean inverted) {
		mController = new AdaptivePurePursuitController(DrivetrainConstants.kPathFollowingLookahead,
				DrivetrainConstants.kPathFollowingMaxAccel, DrivetrainConstants.kNormalLoopsDt, path, inverted, 0);
		mController.update(mRobotState);
		newController = true;
	}

	public void setTrajectoryController(Path path, double lookahead, boolean inverted) {
		mController = new AdaptivePurePursuitController(lookahead, DrivetrainConstants.kPathFollowingMaxAccel,
				DrivetrainConstants.kNormalLoopsDt, path, inverted, 0);
		mController.update(mRobotState);
		newController = true;
	}
	...
	public void setTrajectoryController(Path path, double lookahead, boolean inverted, double tolerance) {
		mController = new AdaptivePurePursuitController(lookahead, DrivetrainConstants.kPathFollowingMaxAccel,
				DrivetrainConstants.kNormalLoopsDt, path, inverted, tolerance);
		mController.update(mRobotState);
		newController = true;
	}
	 ...
	 */
	public Pose getPose() {
		// If drivetrain has not had first update yet, return initial robot pose of
		// 0,0,0,0,0,0
		return mRobotState == null ? new Pose() : mPose;
	}
	 ...
	 * generates a {@link SparkDriveSignal}.
	 */
	...
	public String getStatus() {
		return String.format(
				"Drive State: %s%nOutput Control Mode: %s%nLeft Set Point: %s%nRight Set Point: %s%nLeft Position: %s%nRight Position: %s%nGyro: %s%n",
				mState, mSignal.leftOutput.getControlType(), mSignal.leftOutput.getReference(),
				mSignal.rightOutput.getReference(), mPose.leftEncoderPosition, mPose.rightEncoderPosition,
				mPose.heading);
	}
}
