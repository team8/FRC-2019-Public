...
public class Intake extends Subsystem {
	...
	public void update(Commands commands, RobotState robotState) {
		...
		// The intake macro state has eight possible states. Any state can be
		// transferred to automatically or manually,
		...
		switch (mMacroState) {
			...
			case DOWN_FOR_GROUND_INTAKE:
				boolean isCloseEnoughToGround = Math
						.abs(mIntakeWantedAngle - mRobotState.intakeAngle) < mConfig.acceptableAngularError * 3.0;
				if (isCloseEnoughToGround) { // We are all the way on the ground and should start the intake wheels
					mMacroState = commands.wantedIntakeState = IntakeMacroState.GROUND_INTAKE;
						...
						|| commands.wantedIntakeState == IntakeMacroState.HOLDING_CARGO) { // Cancel the intake and go
																							// back into stowed position
					mMacroState = commands.wantedIntakeState = IntakeMacroState.STOWED;
				}
				break;
			case GROUND_INTAKE:
				if (robotState.hasIntakeCargo) { // Test if ball is all the way in carriage and stop spinning intake
													// wheels
					mMacroState = commands.wantedIntakeState = IntakeMacroState.LIFTING_FROM_GROUND_INTAKE;
				} else if (commands.wantedIntakeState == IntakeMacroState.HOLDING_OUT_OF_WAY
						|| commands.wantedIntakeState == IntakeMacroState.HOLDING_CARGO
						|| commands.wantedIntakeState == IntakeMacroState.DOWN_FOR_GROUND_INTAKE) { // Cancel the intake
																									// and go back into
																									// stowed position
					mMacroState = commands.wantedIntakeState = IntakeMacroState.STOWED;
				}
			...
			case DROPPING_INTO_CARRIAGE:
				if (robotState.hasPusherCargoFar) { // Move arm out of the way for the pusher if it is secured in the
													// carriage
					mMacroState = commands.wantedIntakeState = IntakeMacroState.HOLDING_OUT_OF_WAY;
				} else if (commands.wantedIntakeState == IntakeMacroState.HOLDING_OUT_OF_WAY
						|| commands.wantedIntakeState == IntakeMacroState.DOWN_FOR_GROUND_INTAKE) { // Cancel the
																									// hand-off and move
																									// arm to holding
																									// mid
					mMacroState = commands.wantedIntakeState;
		...
		// 2. Compensate for robot acceleration. Derivation is similar to that for an
		// inverted pendulum,
		// and can be found on slack.
		// 3. Compensate for centripetal acceleration on the arm.
		double arbitraryDemand = mConfig.gravityFF
				* Math.cos(Math.toRadians(robotState.intakeAngle - mConfig.angleOffset))
				+ mConfig.accelerationCompensation * robotState.robotAcceleration
						* Math.sin(Math.toRadians(robotState.intakeAngle - mConfig.angleOffset))
				+ mConfig.centripetalCoefficient * robotState.drivePose.headingVelocity
						* robotState.drivePose.headingVelocity
						* Math.sin(Math.toRadians(robotState.intakeAngle - mConfig.angleOffset));
	...
	public String getStatus() {
		return String.format("Intake State: %s%nOutput Control Mode: %s%nSpark Output: %.2f%nUp Down Output: %s",
				mWheelState, mOutput.getControlType(), mOutput.getReference(), mUpDownState);
	}

	private enum WheelState {
		INTAKING, IDLE, EXPELLING, SLOW, MEDIUM, DROPPING
	}

	private enum UpDownState {
		CUSTOM_ANGLE, ZERO_VELOCITY
	}

	public enum IntakeMacroState {
		STOWED, // Stowed at the start of the match
		DOWN_FOR_GROUND_INTAKE, GROUND_INTAKE, // Getting the cargo off the ground
		...
		HOLDING_CARGO, INTAKING_CARGO, EXPELLING_CARGO, HOLDING_CURRENT_ANGLE
	}
}